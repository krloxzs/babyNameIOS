...
class FavouritesVC: BaseViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
    ...
    @objc func optionButtonPress(sender: UIButton) {
       let babyOBJ = self.babys[sender.tag]
        // create the alert
        let alert = UIAlertController(title: AppStrings.FAV_OPT_BUTTON_TITLE, message: AppStrings.LOGOUT_MESSAGES, preferredStyle: UIAlertController.Style.alert)
        // add the actions (buttons)
        alert.addAction(UIAlertAction(title: AppStrings.FAV_OPT_BUTTON_ONE, style: UIAlertAction.Style.destructive, handler: { action in
            self.realmData.deletObjectFromBD(babyOBJ, success: {
                print("success the baby has been deleted")
                self.getBabysFromDB()
            })
        }))
        alert.addAction(UIAlertAction(title: AppStrings.CANCEL, style: UIAlertAction.Style.cancel, handler: nil))
        // show the alert
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if babys!.count > 0 {
            ...
            case "male","female","unisex":
                ...
                cell.optionsButton.addTarget(self, action: #selector(self.optionButtonPress(sender:)), for: UIControl.Event.touchUpInside)
                return cell
            // ads
            default:
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier: FavCollectionViewCell().reuseIdentifierString(), for: indexPath) as! FavCollectionViewCell
